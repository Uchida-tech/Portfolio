...
RSpec.describe "Api::V1::Current::ActiveRecalls", type: :request do
  describe "GET api/v1/current/active_recalls" do
    subject { get(api_v1_current_active_recalls_path, headers:) }

    let(:headers) { current_user.create_new_auth_token }
    let(:current_user) { create(:user) }
    let(:other_user) { create(:user) }

    before { create_list(:active_recall, 2, user: other_user) }

    context "ログインユーザーに紐づく active_recalls レコードが存在する時" do
      before { create_list(:active_recall, 3, user: current_user) }
    ...
    context "ログインユーザーに紐づく active_recalls レコードが存在しない時" do
      it "空の配列が返る" do
        subject
        res = response.parsed_body
        expect(res).to eq []
  ...
  describe "GET api/v1/current/active_recalls/:id" do
    subject { get(api_v1_current_active_recall_path(id), headers:) }

    let(:headers) { current_user.create_new_auth_token }
    let(:current_user) { create(:user) }

    context ":id がログインユーザーに紐づく active_recalls レコードの id である時" do
      let(:current_user_active_recall) { create(:active_recall, user: current_user) }
    ...
    context ":id がログインユーザーに紐づく active_recalls レコードの id ではない時" do
      let(:other_user_active_recall) { create(:active_recall) }
